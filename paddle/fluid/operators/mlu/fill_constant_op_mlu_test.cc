...
template <typename T>
void Compare(f::Scope* scope, const p::DeviceContext& ctx, std::string op_type,
             float value, std::vector<int64_t> shape) {
  // output
  ...
  auto op = f::OpRegistry::CreateOp(op_type, input, {{"Out", {"Out"}}}, attrs);
  // op runner
